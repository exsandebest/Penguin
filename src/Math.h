...
inline double module(double a) { return (a < 0) ? -a : a; }

inline double ln(double a) {
  double xpt = (a - 1.0) / (a + 1.0);
  double res = 0.0, intermediateRes = -1.0, numerator = xpt, denominator = 1.0;
  xpt *= xpt;
  while (module(res - intermediateRes) > EPS_MACHINE) {
    intermediateRes = res, res += numerator / denominator, numerator *= xpt,
    denominator += 2.0;
  }
  return 2.0 * res;
}
...
inline double exp(double a) {
  double res = 0.0, intermediateRes = -1.0, numerator = 1.0, denominator = 1.0,
         cnt = 1.0;
  while (module(res - intermediateRes) > EPS_MACHINE) {
    intermediateRes = res, res += numerator / denominator, numerator *= a,
    denominator *= cnt, cnt += 1.0;
  }
  return res;
}
